...
public:
    CMainParams(const MainNetOptions& opts) {
        ...
        // BDTC has no public seed nodes yet; add them when the network is deployed.
        vSeeds.clear();
        vFixedSeeds.clear();
...
public:
    CTestNetParams(const TestNetOptions& opts) {
        ...
        genesis = CreateGenesisBlock(1757800001, 0, 0x207fffff, 1, 50 * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();

        vFixedSeeds.clear();
        vSeeds.clear();
        // nodes with support for servicebits filtering should be at the top
        // BDTC testnet has no public seed nodes yet.
